...
public class UpdateJoinEvent implements Listener
{
    ...
    public void onJoin(final PlayerJoinEvent e) {
    	...
    	if (p.hasPermission("nv.update")) {
    		if (MainNightV.getPlugin().getConfig().getBoolean("Update.Enabled") == true){
    		this.checker = new UpdateChecker(MainNightV.plugin);
                        if (this.checker.isConnected()) {
                            if (this.checker.hasUpdate()) {
                                ...
                                p.sendMessage(ChatColor.RED + "Your version: " + MainNightV.plugin.getDescription().getVersion());
                                p.sendMessage(ChatColor.GRAY + "=========================");
                            }
                        }               
       }
